...
The command refuses to run until every table named by `permission.table_names` exists. When Spatie teams are enabled, it also requires the configured `permission.column_names.team_foreign_key` on `roles`, `model_has_roles`, and `model_has_permissions`.

Enable and migrate Spatie teams before using team-scoped role assignments:

```bash
# Set permission.teams to true in config/permission.php first.
php artisan permission:setup-teams
php artisan migrate
```

If this schema is absent, `rbac:reset` exits before any jobs run and instructs the operator to create and run Spatie's teams migration.
...
When Spatie teams are enabled, defined roles remain global. The package synchronizes them with a null team ID so they can be assigned in any tenant. Set the active team before assigning a defined role to a user; Spatie stores the assignment in its team-scoped pivot:
...
Tenant-custom roles are application-managed. Create and assign them while the relevant team is active, and do not register them in `rbac.roles`; `rbac:reset` must not manage or overwrite them. This package does not provide tenant-specific permissions: ability enums and Spatie permission records remain global.
...
4. Run `php artisan rbac:reset` in an environment with the permission tables migrated. For teams, enable Spatie teams and run its teams migration first.
